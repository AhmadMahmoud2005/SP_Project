...
//maps blocks
RectangleShape cblocks[3], wblocks[10], dblocks[11], bblocks[6], menu, maps[4], mapsBack[4];
...
bool play = false, canfight = false, start = true, final = false;
int currenttext = 0, cblockspeed = 3;

//sound effects
Sound menuSound, bullet, jump;;
...
int main() {
	...
	RenderWindow window(VideoMode(WIN_WIDTH, WIN_HEIGHT), "StickFight The Game", Style::Default);
	...
	bfore.setPosition(Vector2f(0, 335));
	ballblocks(bblocks);
...
void bulletsound(Player& player) { // 0 pistol , 1 rifle , 2 mini , 3 RPG 
	...
	else if (player.playertecture == 5) {
		bullet.setBuffer(currentsound[2]);
	}
}

//In game font effects
void setfontfight(Font& font) {
...
void ballblocks(RectangleShape bblocks[]) {
	//ball block1
	bblocks[0] = createRectangle(450, 150, 1500, 890);
	//ball block2
	...
	//ball block5
	bblocks[4] = createRectangle(390, 150, 0, 890);
	//ballground
...
// player properties
void playerproperties() {
	player1.sprite.setPosition(450, 150);
